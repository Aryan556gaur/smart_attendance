...
| Feature | Original | NeuralAttend |
|---------|----------|--------------|
| Interface | CLI / basic Streamlit | Full web dashboard |
| Storage | Pickle files only | SQLite + Pickle + CSV |
| Enrollment | Terminal input + OpenCV window | Browser webcam + form |
| Detection | Keypress (`o`) in OpenCV window | Browser button + auto mode |
| Attendance | CSV per date | DB + CSV + web view + download |
| Students | None | Full CRUD with photos |
| OS | Windows only (win32com) | Cross-platform |
| Architecture | Monolithic scripts | REST API + SPA frontend |

---

## ⚠️ Notes

- **Lighting matters**: Good frontal lighting = better recognition
- **Minimum 5 samples** required to complete enrollment (10 recommended)
- **Confidence threshold**: <40% detections are not marked (configurable in `app.py`)
- The KNN model reloads from pickle on each detect request — for production scale, cache it in memory
- For production deployment, replace SQLite with PostgreSQL and serve frontend via Nginx
